...
test.describe('Homepage', () => {
  ...
  test('@smoke page loads and has the correct title', async ({ page }) => {
    await expect(page).toHaveTitle(/gambling\.com/i);
  });

  test('@smoke logo is visible in the header', async () => {
    await expect(homePage.logo).toBeVisible();
  });

  test('@smoke main navigation links are visible', async () => {
    await expect(homePage.reviewsNavLink).toBeVisible();
  });

  test('@smoke page has a visible heading', async () => {
